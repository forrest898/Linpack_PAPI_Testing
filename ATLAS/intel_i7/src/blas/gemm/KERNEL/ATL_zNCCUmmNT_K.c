...
static void ATL_zJIK0x0x24NT1x1x6_aX_bX
 ...
 */
{
   ...
   const int incAm = 2 - (((lda) << 5)+((lda) << 4)), incAn = -(((Mb) << 1));
   ...
   #undef Kb
#endif
void ATL_zJIK0x0x24NT0x0x0_aX_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=24, 
 ...
 */
{
   ...
   const double BetaAlpha = beta / alpha;
   const int incAk = (((lda) << 1));
   const int incAm = 24 - (((lda) << 5)+((lda) << 4)), incAn = -(((Mb) << 1));
   const int incBk = (((ldb) << 1)), incBm = -(((ldb) << 5)+((ldb) << 4));
   #define incBn 2
   #define incCm 24
   const int incCn = (((ldc) << 1)) - (((Mb) << 1));
   ...
   if (k=M-Mb)
      ATL_zJIK0x0x24NT1x1x6_aX_bX(k, N, 24, alpha, ca + (((Mb) << 1)), lda, cb, ldb, beta, cc + (((Mb) << 1)), ldc);
}
#ifdef incAm
   #undef incAm
#endif
