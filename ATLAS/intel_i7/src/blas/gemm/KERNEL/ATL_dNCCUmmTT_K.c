...
static void ATL_dJIK0x0x48TT1x1x1_aX_bX
 ...
 */
{
   ...
   do /* N-loop */
   {
      ...
      do /* M-loop */
      {
         ...
         rC0_0 = *pC0;
         rC0_0 *= rA0;
         for (k=0; k < 48; k++) /* easy loop to unroll */
         {
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += rA0 * rB0;
 ...
 */
{
   ...
   const double BetaAlpha = beta / alpha;
   #define incAk 1
   const int incAm = ((((lda) << 3)+((lda) << 2)) - 48), incAn = -(Mb*lda);
   const int incBk = (ldb), incBm = -(((ldb) << 5)+((ldb) << 4));
   #define incBn 1
   #define incCm 12
   const int incCn = (ldc) - (Mb);
   ...
   if (k=M-Mb)
      ATL_dJIK0x0x48TT1x1x1_aX_bX(k, N, 48, alpha, ca + (Mb*lda), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
