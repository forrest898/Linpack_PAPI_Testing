...
static void ATL_sJIK0x0x72NN1x1x12_aX_bX
...
/*
 * matmul with TA=N, TB=N, MB=0, NB=0, KB=72, 
 ...
 */
{
   ...
   const int incAk = (lda);
   const int incAm = 1 - (((lda) << 6)+((lda) << 3)), incAn = -(Mb);
   #define incBk 12
   ...
   #undef Kb
#endif
void ATL_sJIK0x0x72NN0x0x0_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=N, MB=0, NB=0, KB=72, 
 ...
 */
{
   ...
   const float BetaAlpha = beta / alpha;
   const int incAk = (lda);
   const int incAm = 12 - (((lda) << 6)+((lda) << 3)), incAn = -(Mb);
   #define incBk 12
   const int incBm = -72, incBn = (ldb);
   #define incCm 12
   const int incCn = (ldc) - (Mb);
   ...
   if (k=M-Mb)
      ATL_sJIK0x0x72NN1x1x12_aX_bX(k, N, 72, alpha, ca + (Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
