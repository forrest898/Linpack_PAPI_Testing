...
static void ATL_zJIK0x0x58TN1x1x6_a1_bX
 ...
 */
{
   #define Mb M
   #define Nb N
   const double *stM = A + (58*(Mb));
   const double *stN = B + (58*(Nb));
   ...
   #undef Kb
#endif
void ATL_zJIK0x0x58TN58x58x0_a1_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=58, 
 ...
 */
{
   ...
   if (k=M-Mb)
      ATL_zJIK0x0x58TN1x1x6_a1_bX(k, N, 58, alpha, ca + (58*(Mb)), lda, cb, ldb, beta, cc + (((Mb) << 1)), ldc);
}
#ifdef incAm
   #undef incAm
#endif
