...
static void ATL_cJIK0x0x48NN1x1x12_aX_bX
 ...
 */
{
   ...
   #undef Kb
#endif
void ATL_cJIK0x0x48NN0x0x0_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=N, MB=0, NB=0, KB=48, 
 ...
 */
{
   ...
   const float BetaAlpha = beta / alpha;
   const int incAk = (((lda) << 1));
   const int incAm = 24 - (((lda) << 6)+((lda) << 5)), incAn = -(((Mb) << 1));
   #define incBk 24
   const int incBm = -96, incBn = (((ldb) << 1));
   #define incCm 24
   const int incCn = (((ldc) << 1)) - (((Mb) << 1));
   ...
   if (k=M-Mb)
      ATL_cJIK0x0x48NN1x1x12_aX_bX(k, N, 48, alpha, ca + (((Mb) << 1)), lda, cb, ldb, beta, cc + (((Mb) << 1)), ldc);
}
#ifdef incAm
   #undef incAm
#endif
