...
static void ATL_sJIK0x0x72TN1x1x12_aX_bX
 ...
 */
{
   ...
   #define incAk 12
   const int incAm = ((lda) - 72), incAn = -(Mb*lda);
   #define incBk 12
   ...
   #undef Kb
#endif
void ATL_sJIK0x0x72TN0x0x0_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=72, 
 ...
 */
{
   ...
   const float BetaAlpha = beta / alpha;
   #define incAk 12
   const int incAm = ((((lda) << 3)+((lda) << 2)) - 72), incAn = -(Mb*lda);
   #define incBk 12
   const int incBm = -72, incBn = (ldb);
   #define incCm 12
   const int incCn = (ldc) - (Mb);
   ...
   if (k=M-Mb)
      ATL_sJIK0x0x72TN1x1x12_aX_bX(k, N, 72, alpha, ca + (Mb*lda), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
